    ...
    <table class="table table-striped table-hover">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <span class="badge bg-secondary">{{ ucfirst(str_replace('_', ' ', $record->collection_method)) }}</span>
                ...
                <td>
                    {{ $record->evangelismLeader->name ?? 'N/A' }}
                </td>
                <td>
                    <small class="text-muted">{{ $record->handover_to_evangelism_at->format('M d, Y H:i') }}</small>
                </td>
                <td>
                    <div class="btn-group btn-group-sm" role="group">
                        <a href="{{ route('secretary.offerings.show', $record->id) }}" class="btn btn-info" title="View Details">
                            <i class="fas fa-eye"></i>
                        </a>
                        @if(auth()->user()->isSecretary() || auth()->user()->isAdmin())
                            <button type="button" class="btn btn-success" onclick="finalizeCommunityOffering({{ $record->id }})" title="Finalize">
            ...
            <form id="finalizeCommunityOfferingForm" method="POST">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="secretary_notes" name="secretary_notes" rows="3" placeholder="Add any notes about finalizing this offering..."></textarea>
                    </div>
                </div>
