                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h1 class="h3 mb-0"><i class="fas fa-door-open me-2 text-primary"></i>Return to Fellowship
                                    Request Details</h1>
                            ...
                            <div>
                                ...
                                @elseif(auth()->user()->isPastor() || auth()->user()->isAdmin())
                                    <a href="{{ route('pastor.return-to-fellowship-requests.pending') }}"
                                        class="btn btn-outline-primary">
                                        <i class="fas fa-arrow-left me-1"></i> Back
                                    </a>
                                @endif
                        ...
                        <div class="row">
                            ...
                            @if($returnToFellowshipRequest->date_of_birth)
                                <div class="col-md-3 mb-2">
                                    ...
                                    <span
                                        class="text-muted">{{ $returnToFellowshipRequest->date_of_birth->format('F d, Y') }}</span>
                                </div>
                            @endif
                        </div>
                        <div class="row">
                    ...
                    <div class="card-body">
                        <div class="row mb-2">
                            <div class="col-md-6">
                                ...
                                <span
                                    class="badge bg-{{ $returnToFellowshipRequest->previously_member ? 'info' : 'secondary' }}">
                ...
                <div class="card border-0 shadow-sm mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            ...
                            <span
                                class="badge bg-{{ $returnToFellowshipRequest->declaration_agreed ? 'success' : 'danger' }}">
            ...
            <div class="col-md-4 mb-4">
                <div class="card border-0 shadow-sm mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <span
                                class="text-muted">{{ $returnToFellowshipRequest->submitted_at->format('F d, Y h:i A') }}</span>
                        </div>
                        @if($returnToFellowshipRequest->reviewed_at)
                            <div class="mb-3">
                                <strong>Reviewed:</strong><br>
                                <span
                                    class="text-muted">{{ $returnToFellowshipRequest->reviewed_at->format('F d, Y h:i A') }}</span>
                            </div>
                        @endif
                        @if($returnToFellowshipRequest->scheduled_counselling_date)
                            <div class="mb-3 p-2 bg-light border border-info rounded">
                                <strong class="text-info"><i class="fas fa-calendar-alt me-1"></i>Scheduled
                                    Counselling:</strong><br>
                                <span
                                    class="text-dark font-weight-bold">{{ $returnToFellowshipRequest->scheduled_counselling_date->format('F d, Y h:i A') }}</span>
                            </div>
                        @endif
                ...
                @if(auth()->user()->isPastor() || auth()->user()->isAdmin())
                    @if($returnToFellowshipRequest->status === 'pending')
                        <div class="card border-0 shadow-sm">
                            ...
                            <div class="card-body">
                                <!-- Approve Form -->
                                <form
                                    action="{{ route('pastor.return-to-fellowship-requests.approve', $returnToFellowshipRequest->id) }}"
                                ...
                                <form
                                    action="{{ route('pastor.return-to-fellowship-requests.require-counseling', $returnToFellowshipRequest->id) }}"
                                    method="POST" class="mb-3">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="scheduled_counselling_date"
                                            class="form-label text-info font-weight-bold">Scheduled Meeting Date & Time <span
                                                class="text-danger">*</span></label>
                                        <input type="datetime-local" class="form-control border-info"
                                            id="scheduled_counselling_date" name="scheduled_counselling_date" required
                                            min="{{ date('Y-m-d\TH:i') }}">
                                    </div>
                                    <div class="mb-3">
                                        <label for="pastor_comments_counseling" class="form-label">Counselling Notes / Comments
                                            <span class="text-danger">*</span></label>
                                        <textarea class="form-control" id="pastor_comments_counseling" name="pastor_comments"
                                            rows="3" required
                                            placeholder="Describe why counselling is needed or provide instructions..."></textarea>
                                    </div>
                                    <button type="submit" class="btn btn-info btn-lg w-100 text-white">
                                        <i class="fas fa-calendar-check me-1"></i> Schedule & Require Counselling
                                    </button>
                                </form>

                                <!-- Reject Form -->
                                <form
                                    action="{{ route('pastor.return-to-fellowship-requests.reject', $returnToFellowshipRequest->id) }}"
                                    ...
                                    <div class="mb-2">
                                        <label for="pastor_comments_reject" class="form-label">Rejection Reason <span
                                                class="text-danger">*</span></label>
                                        <textarea class="form-control" id="pastor_comments_reject" name="pastor_comments" rows="3"
                                            required></textarea>
                                    </div>
                    ...
                    @if($returnToFellowshipRequest->status === 'counseling_required')
                        <div class="card border-0 shadow-sm">
                            <div class="card-header bg-success text-white">
                                <h5 class="mb-0"><i class="fas fa-check-circle me-2"></i>Finalize Request</h5>
                            </div>
                            <div class="card-body">
                                <p class="text-muted small mb-3">After the counselling session is complete, you can provide final
                                    comments and approve the member's return.</p>
                                <form
                                    action="{{ route('pastor.return-to-fellowship-requests.approve-after-counseling', $returnToFellowshipRequest->id) }}"
                                    method="POST">
                                    @csrf
                                    <div class="mb-3">
                                        <label for="final_pastor_comments" class="form-label">Final Pastor Comments
                                            (optional)</label>
                                        <textarea class="form-control" id="final_pastor_comments" name="pastor_comments" rows="3"
                                            placeholder="Enter session outcomes or final notes...">{{ $returnToFellowshipRequest->pastor_comments }}</textarea>
                                    </div>
                                    <button type="submit" class="btn btn-success btn-lg w-100">
                                        <i class="fas fa-user-plus me-1"></i> Complete & Approve Return
                                    </button>
                                </form>
                            </div>
                        </div>
