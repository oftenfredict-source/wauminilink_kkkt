...
@section('content')
                                ...
                                <tbody>
                                    @forelse($contributors as $contribution)
                                        <tr>
                                            ...
                                            <td class="fw-bold text-success">TZS
                                                {{ number_format($contribution->contribution_amount, 2) }}</td>
                                            <td>{{ $contribution->contribution_date ? $contribution->contribution_date->format('M j, Y') : '—' }}
                                            </td>
                                            <td>
                                                <span
                                                    class="badge bg-info">{{ ucfirst(str_replace('_', ' ', $contribution->contribution_type)) }}</span>
                                            </td>
                ...
                <form id="contributionForm">
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="member_id" id="member_id_select" class="form-select" required>
                                ...
                                @foreach($availableMembers as $member)
                                    <option value="{{ $member->id }}" data-member-name="{{ $member->full_name }}"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="contribution_amount" class="form-control" step="0.01" min="0"
                                required>
                        </div>
                        <div class="mb-3">
    ...
    <script>
        ...
        function closeEvent() {
            SwalHelpers.confirm(
                'Close Event',
                'Are you sure you want to close this bereavement event? This action cannot be undone.',
                'Yes, Close',
                'Cancel',
                function () {
            ...
            fetch('{{ route("bereavement.record-contribution", $bereavement->id) }}', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        SwalHelpers.error('Error!', data.message || 'Failed to record contribution');
                    }
                });
        });
    </script>
