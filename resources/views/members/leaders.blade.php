...
@section('content')
                    ...
                    <div class="card-body py-3 px-4">
                        <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="lh-sm">
                                    <h4 class="mb-0 fw-bold text-white" style="color: white !important;">Church Leadership</h4>
                                    <small class="text-white-50">View all church leaders and their contact information</small>
                                </div>
                            </div>
                            @if($memberPositions->count() > 0)
                                <div class="d-flex align-items-center gap-3 header-widgets">
                                    <div class="badge bg-white text-danger px-3 py-2 rounded-pill shadow-sm"
                                        style="font-size: 0.9rem;">
                                        <i class="fas fa-star me-2"></i>
                                        <span class="fw-bold">You are a Leader</span>
                                    </div>
                                </div>
                            @endif
                        </div>
            ...
            <div class="row mb-4">
                <div class="col-12">
                    <div class="card border-0 shadow-sm border-danger"
                        style="border-width: 2px !important; border-radius: 15px;">
                        <div class="card-header bg-white border-0 py-3">
                            <h5 class="mb-0 text-danger fw-bold"><i class="fas fa-user-tie me-2"></i>Your Leadership Positions
                            </h5>
                        </div>
                            ...
                            <div class="row">
                                @foreach($memberPositions as $position)
                                    <div class="col-md-6 mb-3">
                                        <div class="card border-0 shadow-sm rounded-4 position-card h-100"
                                            style="background: #fffafa;">
                                            <div class="card-body p-4">
                                                <div class="d-flex align-items-center">
                                                    <div class="rounded-circle bg-danger text-white d-flex align-items-center justify-content-center me-4 shadow-sm"
                                                        style="width: 70px; height: 70px;">
                                                        <i class="fas fa-star fa-2x"></i>
                                                    </div>
                                                    <div class="flex-grow-1">
                                                        <h4 class="mb-1 text-danger fw-bold">{{ $position->position_display }}
                                                        </h4>
                                                        <div class="d-flex flex-column gap-1">
                                                            <span class="text-dark fw-medium">
                                                                <i class="fas fa-calendar-check me-2 text-danger"></i>
                                                                Appointed: <span
                                                                    class="text-muted">{{ $position->appointment_date->format('d M Y') }}</span>
                                                            </span>
                                                            @if($position->end_date)
                                                                <span class="text-dark fw-medium">
                                                                    <i class="fas fa-calendar-times me-2 text-danger"></i>
                                                                    Term Ends: <span
                                                                        class="text-muted">{{ $position->end_date->format('d M Y') }}</span>
                                                                </span>
                                                            @else
                                                                <span class="text-success small fw-bold">
                                                                    <i class="fas fa-check-circle me-1"></i> Active Member of
                                                                    Leadership
                                                                </span>
                                                            @endif
                                                        </div>
                    ...
                    <div class="card-header bg-secondary text-white">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge bg-warning text-dark fw-bold"
                                style="font-size: 0.875rem; padding: 0.5rem 0.75rem;">{{ $leaders->count() }} Active
                                Leaders</span>
                        </div>
                    </div>
                    <div class="card-body">
                        @if($leaders->count() > 0)
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <input type="text" id="searchLeaders" class="form-control"
                                    ...
                                    <select id="filterPosition" class="form-select">
                                        ...
                                        @foreach($leadersByPosition->keys() as $position)
                                            <option value="{{ $position }}">
                                                {{ $leaders->firstWhere('position', $position)->position_display ?? $position }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                                ...
                                                <td>
                                                    ...
                                                    @if($leader->end_date)
                                                        <br><small class="text-muted">Until:
                                                            {{ is_object($leader->end_date) && method_exists($leader->end_date, 'format') ? $leader->end_date->format('d M Y') : \Carbon\Carbon::parse($leader->end_date)->format('d M Y') }}</small>
                                                    @endif
                                                </td>
                                                <td>
    ...
    <style>
        ...
        .position-card:hover {
            ...
            box-shadow: 0 10px 20px rgba(148, 0, 0, 0.1) !important;
