                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h1 class="h3 mb-0"><i class="fas fa-heart me-2 text-primary"></i>Marriage Blessing Request
                                    Details</h1>
                                <p class="text-muted mb-0">{{ $marriageBlessingRequest->husband_full_name }} &
                                    {{ $marriageBlessingRequest->wife_full_name }}
                                </p>
                            </div>
                            <div>
                                ...
                                @elseif(auth()->user()->isPastor() || auth()->user()->isAdmin())
                                    <a href="{{ route('pastor.marriage-blessing-requests.pending') }}"
                                        class="btn btn-outline-primary">
                                        <i class="fas fa-arrow-left me-1"></i> Back
                                    </a>
                                @endif
        ...
        <div class="row">
            <div class="col-md-8 mb-4">
                ...
                <div class="card border-0 shadow-sm mb-3">
                    ...
                    <div class="card-body">
                        <div class="row">
                            @if($marriageBlessingRequest->marriage_date)
                                <div class="col-md-6 mb-2">
                                    <strong>Marriage Date:</strong><br>
                                    <span
                                        class="text-muted">{{ $marriageBlessingRequest->marriage_date->format('F d, Y') }}</span>
                                </div>
                            @endif
                            @if($marriageBlessingRequest->marriage_type)
                                <div class="col-md-6 mb-2">
                                    ...
                                    <span
                                        class="text-muted">{{ ucfirst(str_replace('_', ' ', $marriageBlessingRequest->marriage_type)) }}</span>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="card-body">
                        <div class="row mb-2">
                            <div class="col-md-6">
                                ...
                                <span
                                    class="badge bg-{{ $marriageBlessingRequest->both_spouses_members ? 'success' : 'warning' }}">
                            ...
                            <div class="col-md-6">
                                ...
                                <span
                                    class="badge bg-{{ $marriageBlessingRequest->attended_marriage_counseling ? 'success' : 'warning' }}">
                ...
                <div class="card border-0 shadow-sm mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            ...
                            <span
                                class="badge bg-{{ $marriageBlessingRequest->declaration_agreed ? 'success' : 'danger' }}">
            ...
            <div class="col-md-4 mb-4">
                <div class="card border-0 shadow-sm mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-3">
                            ...
                            <span
                                class="text-muted">{{ $marriageBlessingRequest->submitted_at->format('F d, Y h:i A') }}</span>
                        </div>
                        @if($marriageBlessingRequest->reviewed_at)
                            <div class="mb-3">
                                <strong>Reviewed:</strong><br>
                                <span
                                    class="text-muted">{{ $marriageBlessingRequest->reviewed_at->format('F d, Y h:i A') }}</span>
                            </div>
                        @endif
                        @if($marriageBlessingRequest->scheduled_meeting_date)
                            <div class="mb-3">
                                <div class="p-2 bg-light rounded border border-primary border-start-4">
                                    ...
                                    <span
                                        class="text-primary fw-bold">{{ $marriageBlessingRequest->scheduled_meeting_date->format('F d, Y h:i A') }}</span>
                                </div>
                            </div>
                        @endif
                        @if($marriageBlessingRequest->scheduled_blessing_date)
                            <div class="mb-3">
                                ...
                                <span
                                    class="text-muted">{{ $marriageBlessingRequest->scheduled_blessing_date->format('F d, Y') }}</span>
                            </div>
                        @endif
                ...
                @if(auth()->user()->isPastor() || auth()->user()->isAdmin())
                        ...
                        <div class="card border-0 shadow-sm mb-3">
                            ...
                            <div class="card-body">
                                <p class="small text-muted mb-3">Schedule an initial meeting with the couple to verify details and
                                    documents.</p>
                                <form
                                    action="{{ route('pastor.marriage-blessing-requests.schedule-meeting', $marriageBlessingRequest->id) }}"
                                    method="POST">
                                    @csrf
                                    <div class="mb-3">
                    ...
                    @if($marriageBlessingRequest->status === 'scheduled' || $marriageBlessingRequest->status === 'pending')
                        <div class="card border-0 shadow-sm mb-3">
                            <div class="card-header bg-primary text-white">
                                <h5 class="mb-0"><i class="fas fa-file-invoice me-2"></i>2. Complete Application</h5>
                            </div>
                            <div class="card-body text-center">
                                <p class="small text-muted mb-3">Fill in the full marriage details, church background, and verify
                                    eligibility.</p>
                                <a href="{{ route('pastor.marriage-blessing-requests.edit', $marriageBlessingRequest->id) }}"
                    ...
                    @if($marriageBlessingRequest->status === 'scheduled' || $marriageBlessingRequest->status === 'pending' || $marriageBlessingRequest->status === 'counseling_required')
                        <div class="card border-0 shadow-sm">
                            <div class="card-header bg-success text-white">
                            ...
                            <div class="card-body">
                                <!-- Approve Form -->
                                <form
                                    action="{{ route('pastor.marriage-blessing-requests.approve', $marriageBlessingRequest->id) }}"
                                    method="POST" class="mb-3">
                                    @csrf
                                    <div class="mb-2">
                                        <label for="scheduled_blessing_date" class="form-label">Scheduled Blessing Date
                                            (optional)</label>
                                        <input type="date" class="form-control mb-2" id="scheduled_blessing_date"
                                            name="scheduled_blessing_date" min="{{ date('Y-m-d', strtotime('+1 day')) }}"
                                            value="{{ $marriageBlessingRequest->scheduled_blessing_date ? $marriageBlessingRequest->scheduled_blessing_date->format('Y-m-d') : '' }}">
                                        ...
                                        <textarea class="form-control" id="pastor_comments_approve" name="pastor_comments"
                                            rows="3">{{ $marriageBlessingRequest->pastor_comments }}</textarea>
                                    </div>
                                    <button type="submit" class="btn btn-success w-100">
                                        <i class="fas fa-check me-1"></i> Approve & Issue Blessing
                                    </button>
                                </form>

                                @if($marriageBlessingRequest->status !== 'counseling_required')
                                    <!-- Require Counseling Form -->
                                    <form
                                        action="{{ route('pastor.marriage-blessing-requests.require-counseling', $marriageBlessingRequest->id) }}"
                                        method="POST" class="mb-3">
                                        @csrf
                                        <div class="mb-2">
                                            <label for="pastor_comments_counseling" class="form-label">Counselling Instructions <span
                                                    class="text-danger">*</span></label>
                                            <textarea class="form-control" id="pastor_comments_counseling" name="pastor_comments"
                                                rows="2" required></textarea>
                                        </div>
                                        <button type="submit" class="btn btn-info w-100 text-white">
                                            <i class="fas fa-user-md me-1"></i> Send to Counseling
                                        </button>
                                    </form>
                                @endif

                                <!-- Reject Form -->
                                <form action="{{ route('pastor.marriage-blessing-requests.reject', $marriageBlessingRequest->id) }}"
                ...
                @if($marriageBlessingRequest->pastor_comments)
                    <div class="card border-0 shadow-sm mt-3">
                        ...
                        <div class="card-body">
                            ...
                            @if($marriageBlessingRequest->pastor && $marriageBlessingRequest->reviewed_at)
                                <small class="text-muted">By: {{ $marriageBlessingRequest->pastor->name }} on
                                    {{ $marriageBlessingRequest->reviewed_at->format('F d, Y h:i A') }}</small>
                            @endif
                        </div>
                    </div>
                @endif
